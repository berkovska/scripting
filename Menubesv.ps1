...
#>
    ...
    try {
        ...
        if ($_.Exception -is [System.OperationCanceledException]) {
            Write-BESVLog "Action cancelled: $Name" WARN
            Write-Host 'Action cancelled. Returning to the menu.' -ForegroundColor Yellow
        } else {
...
do {
    ...
    Write-Host '  3  Base configuration Windows Client: set computer name' -ForegroundColor Magenta
    Write-Host '  4  Base configuration Windows Client: set network' -ForegroundColor Magenta
    ...
    Write-Host ' 16  Join Windows Client to domain' -ForegroundColor Magenta
    ...
    switch ($choice.ToUpper()) {
        ...
        '16' { Invoke-BESVAction 'Join Windows Client to domain' { Join-BESVDomain -Restart } }
