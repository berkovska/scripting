...
#>
    ...
    $inputName = Read-Host "Enter the new computer name, press ENTER for '$xmlName' or type Q to cancel"
    if ($inputName -ieq 'Q') {
        throw [System.OperationCanceledException]::new('Computer name change cancelled by user.')
    }
...
#>
    ...
    if ($domainName -match 'xxx' -or $netbiosName -match 'xxx') {
        throw "Domain.Settings.xml still contains 'xxx'. Change it to BelgoCorpbesv.lab and BelgoCorpbesv before joining the client to the domain."
    }
    ...
    $credential = Get-Credential -Message "Enter domain administrator, for example $netbiosName\Administrator. Click Cancel to return to the menu."
    if (-not $credential) {
        throw [System.OperationCanceledException]::new('Domain join cancelled by user.')
    }
