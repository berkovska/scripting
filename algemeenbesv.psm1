...
.DESCRIPTION
Leest Settings.name uit Computer.Settings.xml. Als de huidige naam al juist is,
wordt alleen gelogd. Als de naam anders is, wordt RunOnce ingesteld zodat het
menu na de herstart opnieuw opent. Optioneel kan automatische aanmelding worden
geconfigureerd.

.PARAMETER ConfigureAutoLogon
Vraagt credentials en configureert tijdelijke autologon.
...
#>
    ...
    $settings = Get-BESVComputerSettings
    $newName = [string]$settings.Settings.name
    if ([string]::IsNullOrWhiteSpace($newName)) { throw 'Computernaam ontbreekt in Computer.Settings.xml.' }
    ...
    if ($ConfigureAutoLogon) {
        $credential = Get-Credential -Message 'Account voor automatische aanmelding na herstart'
        if ($null -eq $credential) {
            Write-BESVLog 'No credentials entered. Computer name change was cancelled to avoid autostart errors.' WARN
            return
        }
        Enable-BESVAutoLogon -Credential $credential
        Write-BESVLog 'Tijdelijke automatische aanmelding ingesteld.' OK
    }

    Register-BESVRunOnce -ScriptPath (Join-Path $global:scriptRoot 'Menubesv.ps1')
    Rename-Computer -NewName $newName -Force
    Write-BESVLog "Computernaam gewijzigd van $env:COMPUTERNAME naar $newName. Herstart nodig." OK
...
Export-ModuleMember -Function Write-BESVLog, Test-BESVAdministrator, Get-BESVComputerSettings, Set-BESVComputerName, Set-BESVNetworkConfiguration, New-BESVFolders, New-BESVShares, Set-BESVShareAndNtfsRights, Enable-BESVAutoLogon, Disable-BESVAutoLogon, Register-BESVRunOnce, Register-BESVStartupTask
