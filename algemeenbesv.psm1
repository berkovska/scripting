...
DHCP of statisch IPv4-adres, gateway en DNS. Met Target Server worden alleen
lan en wan verwerkt. Met Target Client wordt alleen lan2 verwerkt. Zo kan het
zelfde Computer.Settings.xml-bestand gebruikt worden voor Windows Server 2025
en Windows 11.

.PARAMETER Target
Bepaalt welke adapters uit Computer.Settings.xml worden verwerkt.
Server verwerkt lan en wan. Client verwerkt lan2. All verwerkt alle adapters.
...
#>
    ...
    switch ($Target) {
        'Server' { $adapterSettings = $adapterSettings | Where-Object { $_.name -in @('lan', 'wan') } }
        'Client' { $adapterSettings = $adapterSettings | Where-Object { $_.name -eq 'lan2' } }
    }
    ...
    foreach ($adapterSetting in $adapterSettings) {
        ...
        New-NetIPAddress -InterfaceIndex $interfaceIndex -IPAddress $adapterSetting.ip -PrefixLength ([int]$adapterSetting.prefixlength) -ErrorAction Stop | Out-Null
...
Deze functie vervangt een apart RegisterTask.ps1 bestand en wordt vanuit het
hoofdmenu uitgevoerd.
