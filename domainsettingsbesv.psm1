...
#>
    ...
    if ($domainExists) {
        $credential = Get-Credential -Message "Domain administrator for $domainName. Click Cancel to return to the menu."
        if (-not $credential) {
            throw [System.OperationCanceledException]::new('Adding an extra domain controller was cancelled by user.')
        }
