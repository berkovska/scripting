...
#>
    ...
    foreach ($row in $rows) {
        ...
        if (Get-ADGroup -Filter "SamAccountName -eq '$escapedName'" -ErrorAction SilentlyContinue) {
...
#>
    ...
    foreach ($user in $data.users) {
        ...
        if (Get-ADUser -Identity $user.login -ErrorAction SilentlyContinue) {
        ...
        $homeDirectory = "\\$env:COMPUTERNAME\$homeShare\$($user.login)"
        $profilePath = Join-Path $profileRoot $user.login
        $localHome = Join-Path $homeRoot $user.login
        ...
        New-ADUser -Name $displayName -GivenName $user.firstName -Surname $user.lastName -SamAccountName $user.login -UserPrincipalName $upn -Path $ouDn -AccountPassword $password -Enabled $true -ChangePasswordAtLogon $true -HomeDrive $homeDrive -HomeDirectory $homeDirectory -ProfilePath $profilePath -ErrorAction Stop
        Write-BESVLog "Gebruiker aangemaakt: $displayName ($($user.login)) in $ouDn" OK
    }
}
...
#>
    ...
    foreach ($user in $data.users) {
        if (-not (Get-ADUser -Identity $user.login -ErrorAction SilentlyContinue)) {
